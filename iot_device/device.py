...
class Device:
    ...
    def __init__(self, connection):
        ...
        self.__uid = self.eval_func(_uid)
    ...
    def softreset(self):
        ...
        try:
            self.__connection.write(MCU_ABORT)
            self.__connection.write(MCU_RESET)
            self.__connection.write(b'\n')
            self.__connection.read_until(b'raw REPL; CTRL-B to exit\r\n>')
            # successful evaluation implies device is online
            self.seen()   
    ...
    def __repr__(self):
        return f"Device {self.uid}, age={self.age:.1f}s"
    ...
    def __exec_part_1(self, code):
        ...
        self.__connection.write(MCU_ABORT)
        self.__connection.write(MCU_ABORT)
        self.__connection.write(MCU_RAW_REPL)
        self.__connection.read_until(b'raw REPL; CTRL-B to exit\r\n>')
        self.__connection.write(code)
        self.__connection.write(MCU_EVAL)
        # process result of format "OK _answer_ EOT _error_message_ EOT>"
        if self.__connection.read(2) != b'OK':
    ...
    def __exec_part_2(self, output):
        if output:
            ...
            while True:
                ans = self.__connection.read_all().split(EOT)
            ...
            result = bytearray()
            while True:
                result.extend(self.__connection.read_all())
